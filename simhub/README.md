...
엔진 모드로 전체 기능이 도는 상태에서, C# 내장 엔진을 조각씩 채운다
(교살자 패턴 — 다 될 때까지 기다리지 않고 되는 것부터 교체):

1. 이벤트 버스 + 멘트 풀(영어 1262개) + edge-tts 사전 캐시 + 무전기 효과
2. 분석기 이식 (트래픽/레이스컨트롤/차량상태/연료/타이어/페이스/라이벌/전략)
3. LLM 멘트 + REST 보조 소스(가상 에너지/날씨 예보)

각 단계가 끝날 때마다 `builtin` 모드로 확인하고, 다 되면 기본 모드를 바꾼다.
파이썬 버전(저장소 루트)은 계속 레퍼런스 겸 실차 튜닝용으로 유지한다 —
mock 리플레이 회귀가 거기 있어서 버그는 파이썬에서 먼저 잡는 게 빠르다.
